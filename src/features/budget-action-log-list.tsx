...
export type UpdateApprovedBudgetPayload =
  | { budgetType: "OPEX"; budgetAmount: number; remarks: string }
  | {
      budgetType: "CAPEX";
      quantity: number;
      costPerUnit: number;
    ...
    <div>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-foreground/60">
            Current amount {formatRm(amount)}. Status stays approved.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-6 flex flex-col gap-4">
        {isCapex ? (
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="update-amount">OPEX budget (RM)</Label>
            <Input
              id="update-amount"
              inputMode="decimal"
              value={budgetAmount}
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-foreground/10 pt-6">
        <button
          type="button"
          onClick={submit}
          disabled={saving || remarksMissing || amountValue <= 0}
